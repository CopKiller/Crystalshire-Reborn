...
Sub DrawActionMsg(ByVal Index As Integer)
    ...
    Case ACTIONMsgSCROLL
        ...
        ActionMsg(Index).Alpha = ActionMsg(Index).Alpha - 3
                ...
                If IsValidMapPoint(X, Y) Then
                    With Map.TileData.Tile(X, Y)
                        ...
                        Case TILE_TYPE_LIGHT
                            RenderText font(theFont), "L", tx, ty, Gold
                        End Select
                    End With
                End If
            Next
            ...
            If Player(MyIndex).PlayerQuest(i).Status = QUEST_STARTED Then
                If Quest(i).Task(Player(MyIndex).PlayerQuest(i).ActualTask).NPC = NpcNum Then
                    If NPC(NpcNum).Sprite > 0 Then
                        textX = ConvertMapX(MapNpc(Index).X * PIC_X) + MapNpc(Index).xOffset + (PIC_X \ 2) - (mTexture(Tex_GUI(5)).w / 2)
                        textY = ConvertMapY(MapNpc(Index).Y * PIC_Y) + MapNpc(Index).yOffset - (mTexture(Tex_Char(NPC(NpcNum).Sprite)).h / 4)
